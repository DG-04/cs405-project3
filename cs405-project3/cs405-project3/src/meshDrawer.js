...
void main()
{
	...
	diff = max(dot(normal, lightdir), 0.0);  //for the diffuse

	vec3 viewDir = normalize(-fragPos); //normalize the light direction
	vec3 refDir = reflect(-lightdir, viewDir); //for reflect vector
	spec = max(dot(viewDir, refDir), 0.0); //for specular light
